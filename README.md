# Databricks Plugin Marketplace

Public marketplace for Databricks Claude Code plugins.
...
2. Click the `+` to add a marketplace → enter `stromy-org/databricks-marketplace`
3. Click **Databricks** → Install

### Option B: From the CLI

```bash
# Add marketplace (one-time)
claude plugin marketplace add stromy-org/databricks-marketplace

# Install plugin
claude plugin install databricks-plugin@databricks-marketplace
```

### Post-install: dependencies (one-time)

```bash
cd ~/.claude/plugins/cache/databricks-marketplace/databricks-plugin/0.1.0
...
| Skill | Command |
|-------|---------|
| _example_ | `/databricks-plugin:example` |

## Updating

```bash
claude plugin update databricks-plugin@databricks-marketplace
```
...
| "Plugin not found in marketplace" | Run `claude plugin marketplace add stromy-org/databricks-marketplace` first |

## Architecture

- **Marketplace** (this repo): public — hosts `marketplace.json` only
- **Plugin** (`databricks-plugin`): public — contains skills, brand data, company info
- **Source format**: use `"source": "url"` with an explicit HTTPS clone URL ending in `.git`, for example `https://github.com/stromy-org/databricks-plugin.git`
